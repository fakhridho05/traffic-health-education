...
export default function AdminTesHasil() {
  ...
  const filteredResults = useMemo(() => {
    if (filterType === 'all') return currentResults;
    return currentResults.filter(r => r.type === filterType);
  }, [currentResults, filterType]);

  const handleDownloadExcel = () => {
    const workbook = XLSX.utils.book_new();

    // Sheet 1: Hasil Web (Online)
    const onlineData = onlineResults.map((r, i) => ({
      'No': i + 1,
    ...
    // Sheet 2: Hasil Seminar (Offline)
    const offlineData = offlineResults.map((r, i) => ({
      'No': i + 1,
      'Waktu Pengerjaan': r.dateStr,
      'Nama Peserta': r.nama,
    ...
    XLSX.writeFile(workbook, 'Rekap_Hasil_Tes_Lengkap.xlsx');
  };
    ...
    <div className="animate-fade-in" style={{ padding: '10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ color: '#666', fontSize: '0.95rem', marginTop: '4px' }}>Data skor evaluasi edukasi seluruh pengguna</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '20px', borderBottom: '2px solid #e5e7eb' }}>
        <button
          ...
          style={{
            padding: '12px 24px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', border: 'none',
      ...
      <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #eaeaea', overflow: 'hidden' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '650px' }}>
            ...
            <tbody>
              {filteredResults.length === 0 ? (
                <tr>
                  <td colSpan={activeSource === 'offline' ? 6 : 5} style={{ textAlign: 'center', padding: '40px', color: '#aaa' }}>
        ...
        {filteredResults.length > 0 && (
          <div style={{ padding: '16px', borderTop: '1px solid #eee', background: '#fafafa', color: '#666', fontSize: '0.85rem', textAlign: 'center' }}>
            Total Data: <strong>{filteredResults.length}</strong> respons.
          </div>
        )}
      </div>
    </div>
